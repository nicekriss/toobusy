...
- [`wan21_scail2.json`](docs/workflows/wan21_scail2.json) — **Wan 2.1 SCAIL-2 모션 트랜스퍼 풀 그래프(92노드).** 레퍼런스 이미지 + 댄스 영상 → SAM3 세그멘테이션 → 베이스 생성 + 익스텐드 2회. 결과: [`wan21_scail2_sample.mp4`](docs/workflows/wan21_scail2_sample.mp4).
...
```text
reference_1_image / reference_2_image / ...
-> Flux2 Klein
-> image / latent / model / model_clean / clip / vae / positive
```

레퍼런스 슬롯은 최대 5개까지 확장할 수 있습니다. 기본 `size_mode`는 `from reference`라서 reference #1의 크기를 따릅니다.
...
2. Flux2 Klein 예제 워크플로우와 미리보기 이미지 추가.
3. 메인 README는 가볍게 유지하고, 노드별 상세 설명은 `docs/`로 분리.
4. ComfyUI-Manager / Registry 배포 기준 점검.
